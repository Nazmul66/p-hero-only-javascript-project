...
// not understand
const isLiked = (id) => {
    return likedPostsId.length && !!likedPostsId.includes(id); // && ar ager code ta buji ni
};

// addToLiked heart btn and not understand
const addToLiked = (id) => {
    likedPostsId.push(id); 
    showPosts(posts);// prob
};

// reportPost & not understand 
const reportPost = (id) => {
    reportedPostsId.push(id);
    const remainingPosts = posts.filter((post) => !reportedPostsId.includes(post.id)); // !report not understand
    showPosts(remainingPosts);// prob
};
