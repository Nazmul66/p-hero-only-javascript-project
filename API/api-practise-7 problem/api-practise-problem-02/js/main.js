...
function dataFind(data){
  ...
  for(let dataAll of data){
    console.log(dataAll);
    const col = document.createElement("div");
    col.classList.add("col-half");
    col.innerHTML = `
        <img class="mb-5" src="${dataAll.imageURL}" alt="">
        <h3 class="">Car Name: ${dataAll.name} </h3>
        <p class="mb-5">Car details: ${dataAll.description}</p>
        <button class="bg-blue-600 text-white rounded py-1 px-3">Car price: ${dataAll.price}</button>
    `;
